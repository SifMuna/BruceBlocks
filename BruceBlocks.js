...
// order: I, O, T, S, Z, J, L -- rot 0 = spawn, 1..3 clockwise. (x, y) offsets, y down.
var SHAPES = [
...
// NOTE: the well rect (90..150) and both HUD panels (0..87, 152..239) are
// horizontally disjoint by construction - nothing can ever overlap, so the
// HUD and the well are independent redraw domains. Keep it that way.
...
function lockPiece() {
    ...
    // the piece was already painted in this exact color, so board now matches
    // shadow already - locking is a display no-op, nothing to redraw here.
    lockDue = 0;
    clearRows = [];
    for (r = 0; r < ROWS; r++) {
        full = true;
        ...
        if (full) {
            clearRows.push(r);
            // flash bars: the only place besides renderWell() that draws inside
            // the well - finishClear() repairs the shadow desync this causes.
...
function finishClear() {
    ...
    for (k = 0; k < clearRows.length; k++) {
        ...
        // is NOT shifted below, since it tracks the panel, which never shifts.
...
function drawPlay() {
    ...
    drawHud();
    drawNext();
    // no active piece exists during the flash sub-phase - skip or renderWell
    // would index SHAPES with a stale type/rot and paint a phantom.
...
function main() {
    ...
    while (true) {
        ...
        switch (state) {
            ...
            case STATE_PLAY:
                updatePlay();
                // required: updatePlay can reach lockPiece -> spawnPiece -> gameOver,
                // switching state mid-iteration. without this re-check drawPlay would
                // repaint the play screen and mark it static, leaving game-over blank.
